...
```{r ans11}
# loss1Companies
log1 <- finStatement[[16]] <= 0
loss1Companies <- c(finStatement$公司名稱[log1]
)
loss1Companies 
```
### 1.2 每股盈餘
...
```{r ans12}
# loss2Companies
log2 <- finStatement[[33]]<= 0
loss2Companies <- c(finStatement$公司名稱[log2])
loss2Companies
```
### 1.3 單一條件成立
...
```{r ans13}
# only1LossCompanies
log3 <-  (finStatement[[16]] >= 0 ) & (finStatement[[33]] <= 0)
loss3Companies <- c(finStatement$公司名稱[log3])
log4 <- (finStatement[[33]] >= 0) & (finStatement[[16]]<= 0)
loss4Companies <- c(finStatement$公司名稱[log4])
only1LossCompanies <- c(loss3Companies,loss4Companies)
only1LossCompanies
```
...
subsample <- data.frame(
    有營業利益卻出現每股盈餘損失或有營業損失卻出現每股盈餘獲利 = c(only1LossCompanies)
)
subsample
```
...
在執行前述`str(exhibitionList[[1]])`時，會看展覽1的主要資訊。請建立一個data frame class答案物件df_oneShow：
  ...
  * 它只有「六」元素，分別來自於`exhibitionList[[1]]`的title及其showInfo底下的time, location, locationName, price,及endTime，
  
  * 各元素請使用來源元素相同名稱命名，故`names(df_oneShow)`會有time, location, locationName, price,及endTime名稱。
...
```{r ans25}
# totalCount
as.factor(df_allShows$sourceWebName) == c("全國藝文活動資訊系統") -> nu
sum(as.integer(nu)) -> totalCount
totalCount
```
